...
Lifted from ComfyUI-Continuity's `creator/refine_local.py`.

`prompting.py` builds the request and reads the reply; this is the half that
holds the weights. They live in this process on purpose. A second runtime with
its own copy of the model — an Ollama, an llama.cpp server — is VRAM ComfyUI
cannot see or reclaim, and on a machine already streaming H3's own 25 GB encoder
off system RAM that is the difference between a rewrite that takes twenty
seconds and one that takes ten minutes. A text encoder loaded here is an
ordinary entry in ComfyUI's model list — and one that is *released* the moment
its generation ends (see `release`), because the refiner idling in VRAM after
the node has run serves nobody: the H3 sampler downstream of it wants that
space. The weights wait on the offload device instead, so the next run is a
copy, not a download. `remote.py` is the deliberate exception, for the machine
where the argument runs the other way: a server the user already keeps resident
for other work, where the in-process load would be the second copy.
...
Everything except the loading is in `harness.py` and `prompting.py`,
...
def load(name):
    ...
    time the node runs. Keeping the object does not keep the VRAM:
...
def release():
    ...
    correct behaviour for a model that the graph is about to use again and wrong
    for one it is done with: the H3 sampler downstream wants that space, and idle
    VRAM should be nobody's. The weights land back on the offload device, so the
...
#
# The ids are deliberately not a node's. Inside a graph the executing context is
# already this node's and is used unchanged; this name is for a call made from
# outside one, where borrowing whichever node ran last would attribute a minute
# of token generation to something that finished long ago.
PROGRESS_ID = "h3-prompt-refiner"


def _progress_context():
    """A name for this generation's ticks, where there is not one already.

    A refine inside a running graph — which is every press of this node — has a
    real context already, and this must not overwrite it: the token count belongs
    on that node's own bar. The named context is for a call made outside a prompt,
    which is what the progress hook's `PromptServer.last_prompt_id` fallback
    crashes on. Kept because the fallback is still there and still fatal.
    """
    import contextlib
...
def chat(name, system, message, images=(), temperature=0.7, seed=-1, max_tokens=None,
    ...
    try:
        ...
        # not the weights. See `release`: the refiner does not idle in VRAM.
        release()
