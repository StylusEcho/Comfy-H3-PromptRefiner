...
Lifted from ComfyUI-Continuity's `creator/refine_remote.py`, with one thing
changed: where the key comes from. See below.

`local.py` is the default for a reason it states itself — a second runtime is
VRAM ComfyUI cannot see or reclaim. This module is for the machine where that
argument runs the other way: an LM Studio or Ollama that is already resident for
other work, a box on the LAN with the big model, a hosted API. For that machine
the *in-process* load is the redundant copy.
...
**Where the key lives, and where it never goes.** Upstream a save route wrote
it into its own file in ComfyUI's user directory, owner-only, and the browser
could never read it back. There is no route here and no panel, so the key comes
from the environment — `H3_REFINER_API_KEY` by default, or whatever variable the
node names — and the file is still read when it exists, so an installation that
already had one keeps working. What the key must never be is a node widget: a
widget's value is saved into the workflow `.json`, and a workflow is a thing
people hand around, paste into issues and publish beside a render. The node
carries the *name* of a variable at most. (MITRE's names for the traps this is
built around: CWE-522/312 for a key at rest where a reader can reach it,
CWE-319 for one on plaintext HTTP, CWE-532 for one in a log line, CWE-918 for a
server that will attach it to any URL a request names.)
...
from . import harness as refine

# Its own file, not a corner of anyone's settings: a settings file is quoted in
# bug reports and synced by people who sync their user folder, and a credential
# in it would be one screenshot from public. Owner-only on every write;
# `refiner` is a namespace so a later credential has a home.
FILE = "h3refiner.credentials.json"

# Where the key comes from when no file holds one. Named rather than hardcoded
# at the call site so the node can point at a second variable for a second
# provider without either of them ever being a widget value.
KEY_ENV = "H3_REFINER_API_KEY"

# ...and the URL, for the installation that would rather configure the endpoint
# the same way. The node's own widget wins over both; this is the fallback for
# a workflow that should not carry an address either.
URL_ENV = "H3_REFINER_BASE_URL"
...
# What a fetched error body may contribute to the message raised out of here.
...
def status():
    """What may be said out loud: the URL, and *whether* a key is set.

    Never the key, not even a masked suffix. This is what a node reports when
    somebody asks it what it is pointed at.
    """
    stored = _read()
    return {"url": stored.get("url", ""), "key_set": bool(stored.get("key"))}


def endpoint(url="", key_env=KEY_ENV):
    """Where this run talks, and what it authenticates with -> `(url, key)`.

    Three sources, in the order a person would expect them to win. The `url`
    argument is the node's own widget and outranks everything: it is the one a
    reader of the workflow can see. `URL_ENV` is next, for the installation that
    would rather the address were not in the workflow either. The stored file is
    last, and is what an installation that already had one keeps working from.

    The key is never any of those things. It comes from `key_env`, and failing
    that from the file — where it is still bound to the URL it was saved with,
    exactly as `configure` left it: a stored key is handed over only while the
    endpoint is the one it was earned for.

    **An environment key is bound too, when there is something to bind it to.**
    ComfyUI's server has no authentication, so anyone who can queue a prompt can
    also edit the URL widget on the node — which would otherwise be enough to
    deliver the key to a host of their choosing. Setting `URL_ENV` alongside the
    key is the gesture that pins it: the key then travels to that address and
    nowhere else, and a widget pointing somewhere else is served with no key
    rather than silently redirected. With `URL_ENV` unset there is nothing to
    pin against and the key goes wherever the endpoint resolves to, which is the
    right answer for the loopback LM Studio that has no key worth pinning and
    the wrong one to rely on for a hosted provider.
    """
    stored = _read()
    pinned = normalize_url(os.environ.get(URL_ENV, ""))
    url = normalize_url(url) or pinned or stored.get("url", "")

    key = (os.environ.get(key_env or "", "") or "").strip()
    if key:
        if pinned and url != pinned:
            raise refine.RefineError(
                f"{key_env} is pinned to {pinned} by {URL_ENV}, and this node "
                f"points at {url or 'nothing'} — point it back, or clear {URL_ENV} "
                f"to let the key follow the node"
            )
        return url, key
    if url and url == stored.get("url"):
        return url, stored.get("key", "")
    return url, ""
...
def list_models(url=None, key=None):
    ...
    Called with nothing, it lists for the resolved endpoint. Called with an
    explicit URL by `configure`'s connection test, before anything is stored.
    """
    if url is None:
        url, key = endpoint()
    if not url:
...
def unload(model, url=None, key=""):
    """Ask the server to drop `model`. -> the path that worked, or "".
    ...
    run succeeded; the memory is a preference.
    """
    if url is None:
        url, key = endpoint()
...
def to_data_url(image):
    """A PIL image -> a `data:` URL, downscaled like `local.to_tensor`.
...
def chat(model, system, message, images=(), temperature=0.7, seed=-1,
    ...
    The same signature as `local.chat`, which is what lets `request.py` hold one
    call site for both backends. Two deliberate differences under it. `images`
    ...
    if not url:
        raise refine.RefineError(
            f"no refiner server is configured — give the node a base_url, or set "
            f"{URL_ENV}")
    ...
    if finish == "length":
        raise refine.RefineError(
            ...
            f"cut off. Raise `reply_tokens`, or ask for fewer shots."
        )
    return content
