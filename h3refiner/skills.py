...
Lifted from ComfyUI-Continuity's `creator/refine_skill.py`.

The built-in refiner is a harness: `prompting.py` writes the rules, strips the
guides and dictates a JSON reply, and `contextir.py` assembles the prompt around
the model's prose. A skill is the opposite bet — a `.skill` package
...
harness is exactly what this mode exists to find out, so none of the built-in
rules and no reply contract may leak into it.

Skills are written against H3's document form, and the loader hands the package
over whole and judges nothing: what a skill returns is what leaves this node.
...
back to `@handles` by `refine.normalize_handles` exactly as the harness's output
is, and substituted forward again on the way out — one representation, whichever
mode wrote it.
...
built-in harness — its rules, its guides, its JSON contract — standing and joins
...
(`mode: replace`), and the node's `skill_mode` widget overrides it either way.

No torch, no ComfyUI: everything here is ordinary data and is unit-tested that
way. `harness.py` is where the few shared functions used below come from.
"""
...
def entries():
    ...
    The node's widget draws off this rather than off bare names, because the two
    kinds read differently and start in different modes. Listed by what is on
    disk rather than validated here — `load` is where a broken package becomes a
    message.
    """
...
def load(name):
    ...
    caller may override it, and the node's `skill_mode` widget does.
    """
    # The name becomes a path component, so it is held to what `list_skills` can
    # produce: one plain filename, no separators, not hidden. Anything else is
    # someone probing the filesystem, not a skill — and a node's widget value is
    # whatever the workflow `.json` says it is.
...
def instructions(skill):
    ...
    is, with no header bolted onto it: in `add` mode this text lands inside the
    built-in system prompt, and a `========== SKILL.md ==========` line in the
    middle of it would be the loader talking over the user.
    """
    if not skill["files"]:
        return skill["body"]
    parts = [f"========== {SKILL_MD} ==========\n{skill['body']}"]
...
def parse_reply(content):
    ...
    skill's output and is not judged here — the node returns the prose beside its
    notes, and `refine.check` reports what points at nothing.
    """
